...
export function HotkeyEditor({ hotkeyName, className }: HotkeyEditorProps) {
	...
	const assignAndStopEditing = useCallback(
		async (key: number) => {
			console.log(
			...
			if (result.success) {
				...
				if (result.error?.startsWith(kHotkeyUsedErrorPrefix)) {
		...
		[ctrl, shift, alt, error, assign, resetEditing],
	);
	...
	return (
		<div
			className={classNames("HotkeyEditor", className, {
				editing,
				error: !!error,
				placeholder: keysText === kPlaceholderText,
			})}
