...
class BuildStorageService {
  ...
  async reorderBuildSets(updates: Array<{ id: string; order: number }>): Promise<void> {
    await this.#client.mutation(api.buildSets.reorder, {
      updates: updates.map(({ id, order }) => ({ id: this.#id(id), order })),
    });
  }
