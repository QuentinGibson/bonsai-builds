...
const makeHTMLTemplate = ({
	htmlWebpackPlugin,
}: Record<string, any>) => `<!doctype html>
<html>
  <head>
    <meta charset="UTF-8" />
...
const makeHTMLPlugin = (chunk: string) =>
...
export const makeConfig = (env: EnvArg): Configuration => {
	...
	const entries = kWindowNames.reduce(
		(acc, name) => {
			acc[name] = `./src/${name}`;
			return acc;
		},
	...
	return {
		...
		output: {
			...
			assetModuleFilename: (pathData) => {
				...
				const name = basename(pathData.filename),
		...
		module: {
			rules: [
				...
				{
					test: /\.css$/,
					use: [MiniCssExtractPlugin.loader, "css-loader", makePostCSSLoader()],
				},
				{
					test: /\.s[ac]ss$/i,
					use: [
