...
**Database:** None — `additional_text` lives on node objects in `allocatedNodes` after #001.

**Routes:** No new mutations needed — `breakpoints.update` already accepts the full `allocatedNodes` array. The UI patches the relevant node object in the array and calls `update`.

**UI:**
- Clicking an allocated passive node opens an inline or panel editor for `additional_text`
- The editor uses the same markup textarea, live preview, and tag toolbar already built for inventory slot and skill notes
- Saving patches the node object in `allocatedNodes` (field absent when note is empty — not stored as `""`)
...
- [ ] Saving a note updates the node's `additional_text` in the `allocatedNodes` array
...
- #001 (allocatedNodes must be objects with `additional_text` support)
